...
public class Metodos {
    ...
    public static void dibujarString(BufferedImage icono, int x, int y, String text, JPanel panel) {
        x = x + (icono.getWidth() / 4);
        y = y + (icono.getHeight() / 2);
        Graphics g = panel.getGraphics();
    ...
    public static boolean sePuedeDibujar(int x, int y, int mediaMedida, int ancho, int alto) {
        return x > mediaMedida && y > mediaMedida && x < ancho - mediaMedida && y < alto - mediaMedida;
    }

    public static boolean colisiona(int x, int y, int medida, ArrayList<Nodo> lista) {
        int distE;
        for (Nodo nodo : lista) {
    ...
    public static Nodo seleccionar(int x, int y, int medida, ArrayList<Nodo> lista) {
        int distE;
        for (Nodo nodo : lista) {
            distE = (int) Math.sqrt((int) (Math.pow((nodo.getX() - x), 2)) + (int) (Math.pow((nodo.getY() - y), 2)));
            if (distE < medida / 2) {
